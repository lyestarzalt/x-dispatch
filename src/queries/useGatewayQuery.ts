...
function getRecommendedScenery(airport: GatewayAirport | null): GatewayScenery | null {
  if (!airport?.sceneryPacks) return null;
  return airport.sceneryPacks.find((s) => s.sceneryId === airport.recommendedSceneryId) || null;
}

function formatGatewayDate(dateStr: string | null): string {
