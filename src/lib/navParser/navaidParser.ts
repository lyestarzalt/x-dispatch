...
/**
 * Convert frequency to display format
 * NDB: kHz (e.g., 350)
 * VOR/DME: MHz (e.g., 115.90)
 */
export function formatFrequency(navaid: Navaid): string {
  if (navaid.type === 'NDB') {
    return `${navaid.frequency} kHz`;
  }
  // VOR/DME frequencies are stored as MHz * 100 (e.g., 11590 = 115.90)
  const mhz = navaid.frequency / 100;
  return `${mhz.toFixed(2)} MHz`;
}

/**
 * Filter navaids by type
 */
export function filterNavaidsByType(navaids: Navaid[], types: NavaidType[]): Navaid[] {
  return navaids.filter((n) => types.includes(n.type));
}

/**
 * Get VORs (VOR, VORTAC, VOR-DME)
 */
export function getVORs(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['VOR', 'VORTAC', 'VOR-DME']);
}

/**
 * Get NDBs
 */
export function getNDBs(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['NDB']);
}

/**
 * Get DMEs (standalone only, not co-located with VOR)
 */
export function getDMEs(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['DME', 'TACAN']);
}

/**
 * Get ILS/Localizers
 */
export function getILS(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['ILS', 'LOC']);
}

/**
 * Get Glideslopes
 */
export function getGlideslopes(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['GS']);
}

/**
 * Get Markers (OM, MM, IM)
 */
export function getMarkers(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['OM', 'MM', 'IM']);
}

/**
 * Get FPAP (Final Approach Path Alignment Points)
 */
export function getFPAPs(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['FPAP']);
}

/**
 * Get GLS (GBAS Landing System stations)
 */
export function getGLS(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['GLS']);
}

/**
 * Get LTP/FTP (Landing/Fictitious Threshold Points)
 */
export function getLTPFTP(navaids: Navaid[]): Navaid[] {
  return filterNavaidsByType(navaids, ['LTP', 'FTP']);
}

/**
 * Get all ILS components for a specific airport/runway
 */
export function getILSComponents(navaids: Navaid[], airport: string, runway?: string): Navaid[] {
  return navaids.filter((n) => {
    const isILSType = ['ILS', 'LOC', 'GS', 'OM', 'MM', 'IM'].includes(n.type);
    const matchesAirport = n.associatedAirport === airport;
    const matchesRunway = !runway || n.associatedRunway === runway;
    return isILSType && matchesAirport && matchesRunway;
  });
}

/**
 * Get all approach aids for a specific airport (ILS, GLS, FPAP, LTP)
 */
export function getApproachAids(navaids: Navaid[], airport: string, runway?: string): Navaid[] {
  return navaids.filter((n) => {
    const isApproachType = [
      'ILS',
      'LOC',
      'GS',
      'OM',
    ...
    const matchesAirport = n.associatedAirport === airport;
    const matchesRunway = !runway || n.associatedRunway === runway;
    return isApproachType && matchesAirport && matchesRunway;
  });
}
