...
const TRAIL_LAYER_ID = 'ivao-trails';
const TRAIL_SOURCE_ID = 'ivao-trails-source';
const LABEL_LAYER_ID = 'ivao-labels';
...
function createTrailGeoJSON(pilots: IvaoPilot[]): GeoJSON.FeatureCollection {
  return {
    type: 'FeatureCollection',
    features: pilots
...
export function removeIvaoPilotLayer(map: maplibregl.Map): void {
  const doRemove = () => {
  ...
  map.on('click', PILOT_LAYER_ID, (e) => {
    ...
    const html = `
      <div class="bg-card text-foreground p-4 rounded-lg border border-cat-blue/30 min-w-[260px] shadow-xl">
        ...
        <div class="mt-3 pt-2 border-t border-border text-sm text-muted-foreground">IVAO ID: ${props.userId}</div>
      </div>
    `;
