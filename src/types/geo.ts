...
interface GeoPoint extends Coordinates {
  elevation?: number;
}
...
function coords(latitude: number, longitude: number): Coordinates {
  return { latitude, longitude };
}

function distanceNm(from: Coordinates, to: Coordinates): number {
  const R = 3440.065;
