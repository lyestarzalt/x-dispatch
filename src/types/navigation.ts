...
function formatNavaidFrequency(navaid: { type: string; frequency: number }): string {
  if (navaid.type === 'NDB') {
...
function getNavaidColor(type: string): string {
  switch (type) {
    case 'VOR':
    case 'VORTAC':
    case 'VOR-DME':
...
function getAirspaceColor(airspaceClass: string): {
  fill: string;
  border: string;
  opacity: number;
} {
